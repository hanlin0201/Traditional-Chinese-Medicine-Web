...
const TCM_CHAT_URL =
  "https://htrtcaswqydnfvgwernh.supabase.co/functions/v1/tcm-chat";
...
【非问诊输入识别】
当用户输入的内容与身体症状、健康问题**完全无关**时（如打招呼、闲聊、测试文字、无意义输入、表达情绪等），**禁止**触发问诊流程，**禁止**附带 inquiry 卡片。直接用一两句自然语言友好回应即可，例如"您好！有什么中医问题可以告诉我，我来帮您。"。只有当用户明确描述症状、身体不适或主动询问中医健康相关内容时，才进入问诊流程。
...
const WELCOME_MSG = {
  ...
  type: "text",
  content:
    "您好，我是 AI 养生导师。请告诉我您哪里不舒服，或者有什么中医药知识问题都可以咨询我。",
};
...
function persistSessions() {
  try {
    localStorage.setItem(getSessionsKey(), JSON.stringify(sessions.value));
  } catch {}
}
...
function saveActiveSession() {
  ...
  // 用第一条用户消息自动生成标题
  if (
    !sessions.value[idx].manualTitle &&
    sessions.value[idx].title === "新对话"
  ) {
...
function deleteSession(id, e) {
  ...
  if (activeSessionId.value === id) {
    if (sessions.value.length) {
      ...
      messages.value = sessions.value[0].messages?.length
...
async function callSiliconFlowStream(onChunk, userInputText = "") {
  ...
  if (lastPrescriptionIdx >= 0 && apiMessages.length > 0) {
    apiMessages.unshift({
      role: "assistant",
      content:
        "【系统提示】本对话有过往问诊记录。若用户提及上次处方或症状变化，请结合历史信息综合判断，无需重复追问已了解的内容。若用户提出全新问题，则重新独立辨证分析。",
    });
  }

  // 🚀 4. userTurnCount 统计【当前新轮次】中 AI 实际发出的 inquiry 卡片数量
  // 只有真实问诊来回才计数，用户问知识问题时 AI 不生成 inquiry 卡片，不影响轮数
  const currentStartIndex =
    lastPrescriptionIdx >= 0 ? lastPrescriptionIdx + 1 : 0;
  const currentTurnCount = list
    .slice(currentStartIndex)
    .filter((m) => m.type === "inquiry").length;
  ...
  const response = await fetch(TCM_CHAT_URL, {
    ...
    body: JSON.stringify({
      ...
      userJustConfirmed: CONFIRM_KEYWORDS.some((k) =>
        userInputText.includes(k),
      ),
      conversationResetAfterPrescription: lastPrescriptionIdx >= 0,
      isGeneralQuestion: GENERAL_QUESTION_PATTERNS.some((k) =>
        userInputText.includes(k),
      ),
...
function extractJSON(text, preferType = null) {
  ...
  for (const block of codeBlocks) {
    ...
    if (parsed && parsed.type && (!preferType || parsed.type === preferType))
      return parsed;
  }
  const rx =
    preferType === "prescription"
      ? /"type"\s*:\s*"prescription"/
      : /"type"\s*:\s*"(?:prescription|inquiry)"/;
      ...
      for (let i = start; i < text.length; i++) {
        ...
        if (depth === 0) {
          ...
          if (
            parsed &&
            parsed.type &&
            (!preferType || parsed.type === preferType)
          )
            return parsed;
          break;
        }
...
function stripJsonFromText(text) {
  ...
  result = result
    .replace(/\s*type\s*:\s*["']?(?:inquiry|prescription)["']?\s*/gi, "")
    .trim();
  return result.trim();
}
...
function isUnsafeRecipe(r) {
  ...
  if (
    (r.name?.includes("丸") || r.name?.includes("散")) &&
    (stepsStr.includes("成药") || stepsStr.includes("可直接购买"))
  )
    return true;
  return false;
}
...
function cleanPrescription(jsonData) {
  ...
  if (!Array.isArray(aiResponse.recipes)) {
    aiResponse.recipes = Array.isArray(jsonData.recipe)
      ? jsonData.recipe
      : jsonData.recipes
        ? [jsonData.recipes].flat()
        : [];
  }
  aiResponse.recipes = aiResponse.recipes
    .map((r) => {
      if (typeof r === "string") {
        return {
          name: r,
  ...
  try {
    const fullText = await callSiliconFlowStream((chunk, accumulated) => {
      ...
      for (let i = 0; i < textLines.length; i++) {
        ...
        if (
          tl.startsWith("{") &&
          (tl.includes('"type"') || tl.includes('"prescription"'))
        ) {
          messages.value[streamIdx].content = textLines
            .slice(0, i)
            .join("\n")
            .trim();
          if (accumulated.includes('"prescription"')) {
    ...
    if (jsonData && jsonData.type === "prescription") {
      ...
      messages.value[streamIdx] = {
        role: "assistant",
        type: "text",
        content: stripJsonFromText(fullText) || "...",
      };
    }
  } catch (err) {
    console.error(err);
...
function confirmSelection(msg) {
  ...
  input.value = msg.selectedOptions.join("、");
  send();
}
...
async function handleSavePlan(fullMessage) {
  ...
  if (planToSave.recipes && Array.isArray(planToSave.recipes)) {
    planToSave.recipes = planToSave.recipes.filter((r) =>
      favorited.value.has(String(r.id)),
    );
  }
  ...
  try {
    const { data } = await supabase
      ...
      .eq("id", user.value.id)
      .single();
    let list = data?.care_plans || [];
    if (!Array.isArray(list)) list = [];

    const planKey = (p) => {
      const { id, saved_at, ...rest } = p;
    ...
    await supabase
      .from("profiles")
      .update({ care_plans: [newPlan, ...list] })
      .eq("id", user.value.id);
    window.dispatchEvent(new Event("profile-updated"));
    ...
    messages.value.push({
      role: "assistant",
      type: "text",
      content: `✅ 已加入调理计划（包含 ${count} 个已收藏食谱）。`,
    });
  } catch (err) {
    console.error(err);
    messages.value.push({
...
function persistPanelSize() {
  ...
  try {
    localStorage.setItem(
      AI_PANEL_SIZE_KEY,
      JSON.stringify({ w: panelW.value, h: panelH.value }),
    );
  } catch {}
}
...
function onResizeHandleDown(e) {
  ...
  const onMove = (ev) => {
    ...
    panelW.value = Math.max(
      PANEL_MIN_W,
      Math.min(PANEL_MAX_W, Math.min(nextW, window.innerWidth - 24)),
    );
...
<template>
  <button
    class="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-sandalwood text-white shadow-card flex items-center justify-center hover:bg-sandalwood/90 transition-all"
    @click="toggle"
  >
  ...
  <Teleport to="body">
    ...
    >
      ...
      >
        ...
        <div
          class="px-4 py-3 border-b border-sandalwood/15 flex justify-between items-center bg-[#FBF8F2] shrink-0"
        >
          <div class="max-w-[55%] flex items-center min-w-0">
            ...
            <template v-else>
              ...
              >
                ...
                <Edit2
                  class="w-3.5 h-3.5 text-sandalwood/40 group-hover:text-sandalwood transition-colors opacity-0 group-hover:opacity-100 shrink-0"
                />
              </div>
            </template>
          </div>
          <div class="flex items-center gap-1.5">
            <button
              ...
              class="p-1.5 rounded-lg hover:bg-sandalwood/10 text-sandalwood/60 hover:text-sandalwood transition disabled:opacity-30 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              title="历史对话"
              class="p-1.5 rounded-lg hover:bg-sandalwood/10 transition"
              :class="
                showSessionList
                  ? 'text-sandalwood bg-sandalwood/10'
            ...
            <X
              class="w-5 h-5 cursor-pointer text-sandalwood/70 ml-1"
              @click="open = false"
            />
          </div>
        </div>
        ...
        <div
          ...
          class="absolute left-0 right-0 bottom-0 z-10 bg-[#FDFBF7] flex flex-col border-t border-sandalwood/15 overflow-hidden"
          style="top: 52px"
        >
          <div
            class="px-4 py-2.5 border-b border-sandalwood/10 flex items-center justify-between shrink-0"
          >
            <span
              class="text-xs font-semibold text-sandalwood/70 uppercase tracking-wide"
              >历史对话</span
            >
            <button
              v-if="user"
              @click="createNewSession"
              :disabled="isGenerating"
              class="flex items-center gap-1 text-xs text-sandalwood bg-sandalwood/10 px-2.5 py-1 rounded-lg hover:bg-sandalwood/20 transition disabled:opacity-30 disabled:cursor-not-allowed"
            >
              <PlusCircle class="w-3.5 h-3.5" /> 新建对话
            </button>
          </div>
          <div
            class="ai-panel-scroll flex-1 overflow-y-auto"
          ...
          >
            ...
            <div
              ...
              :class="[
                s.id === activeSessionId ? 'bg-sandalwood/10' : '',
            ...
            >
              ...
              <div class="flex-1 min-w-0">
                ...
                <div
                  v-else
                  class="group/title flex items-center gap-1 min-w-0"
                  @click.stop="startRenameSession(s, $event)"
                >
                  <p class="text-sm text-gray-800 truncate">{{ s.title }}</p>
                  <Edit2
                    class="w-3 h-3 text-sandalwood/30 group-hover/title:text-sandalwood transition-colors opacity-0 group-hover/title:opacity-100 shrink-0"
                  />
                </div>
        ...
        <div
          ...
          class="ai-panel-scroll flex-1 overflow-y-auto p-4 space-y-4 bg-[#FAF6ED]"
          data-scrollable="true"
        >
          <div v-for="(m, i) in messages" :key="i">
            <div
              v-if="m.type === 'text' || !m.type"
              :class="[
                'max-w-[85%] rounded-lg px-4 py-2.5 text-sm',
                m.role === 'user'
                  ? 'ml-auto bg-sandalwood/15 text-sandalwood'
                  : 'mr-auto bg-[#F5EDD8] text-sandalwood whitespace-pre-wrap',
              ]"
            >
              {{
                m.role === "user"
            ...
            <div
              v-else-if="
                m.type === 'streaming' && (m.content || !prescriptionLoading)
              "
              class="mr-auto bg-[#F5EDD8] text-sandalwood px-4 py-2.5 rounded-lg text-sm max-w-[85%] whitespace-pre-wrap"
            >
              <span>{{ cleanDisplayContent(m.content) }}</span
              ><span
                class="inline-block w-0.5 h-3.5 bg-sandalwood/70 ml-0.5 animate-blink align-middle"
              ></span>
            </div>

            <div
              v-else-if="m.type === 'inquiry'"
              class="mr-auto bg-white border border-sandalwood/20 p-4 rounded-xl shadow-sm w-[90%] animate-slide-up"
            >
              ...
              <div class="flex flex-col gap-2">
                <button
                  ...
                  :class="[
                    m.selectedOptions && m.selectedOptions.includes(opt.label)
                      ? 'bg-sandalwood text-white border-sandalwood'
                      : opt.value === 'finish'
                        ? 'bg-gray-100 text-gray-700 font-bold border-gray-300'
                        : 'bg-white border-sandalwood/30 text-gray-700 hover:bg-sandalwood/5',
                  ]"
                >
                  <span
                    :class="{ 'italic opacity-80': opt.value === 'none' }"
                    >{{ opt.label }}</span
                  >
              ...
              <div
                v-if="m.selectedOptions && m.selectedOptions.length > 0"
                class="mt-3 pt-3 border-t border-gray-100"
              >
                <button
                  @click="confirmSelection(m)"
                  class="w-full bg-sandalwood text-white py-2 rounded-lg text-xs font-bold flex justify-center items-center gap-1 hover:bg-sandalwood/90"
                >
                  选好了，发送 ({{ m.selectedOptions.length }})
                  <Send class="w-3 h-3" />
                </button>
              </div>
            </div>
            ...
            <div
              v-else-if="m.type === 'prescription'"
              class="mr-auto w-[98%] space-y-2 animate-slide-up"
            >
              <div
                class="bg-[#F5EDD8] text-sandalwood px-4 py-2.5 rounded-lg text-sm mr-auto flex items-start gap-2"
              >
              ...
              <div
                class="bg-white border border-sandalwood/20 rounded-xl overflow-hidden shadow-md"
              >
                <div class="bg-[#FAF6ED] p-3 border-b border-sandalwood/10">
                  <div
                    class="text-sandalwood font-serif font-bold flex items-center gap-2"
                  >
                    💡 {{ m.diagnosis_result }}
                  </div>
                ...
                <div class="p-3 space-y-3">
                  <div
                    v-if="!m.recipes || m.recipes.length === 0"
                    class="text-xs text-amber-600 bg-amber-50 p-2 rounded border border-amber-100"
                  >
                    该方案涉及需专业指导的药材，部分方剂已隐藏，建议线下咨询医师。
                  </div>
                  <div
                    v-for="r in m.recipes"
                    :key="r.id"
                    class="border border-gray-100 rounded bg-white overflow-hidden"
                  >
                    <div
                      @click="foldedStates[r.id] = !foldedStates[r.id]"
                      class="p-2 flex justify-between cursor-pointer hover:bg-gray-50 items-center"
                    >
                      <div class="flex items-center gap-2">
                        ...
                        <div
                          v-else-if="r.category === 'meal'"
                          class="w-6 h-6 rounded-full bg-green-100 text-green-600 flex items-center justify-center"
                        >
                          <Soup class="w-3.5 h-3.5" />
                        </div>
                        <div
                          v-else
                          class="w-6 h-6 rounded-full bg-purple-100 text-purple-600 flex items-center justify-center"
                        >
                      ...
                      <div class="flex items-center gap-1">
                        <span
                          v-if="!foldedStates[r.id]"
                          class="text-[10px] text-sandalwood/60"
                          >点击展开做法</span
                        >
                        <ChevronDown
                          class="w-4 h-4 text-gray-400"
                          :class="{ 'rotate-180': foldedStates[r.id] }"
                        />
                      </div>
                    </div>
                    <div
                      v-show="foldedStates[r.id]"
                      class="p-2 pt-0 text-xs text-gray-600 border-t border-gray-50"
                    >
                      ...
                      <p class="mt-2">
                        <strong>食材：</strong
                        >{{ ensureArray(r.ingredients).join("、") }}
                      </p>
                      <p class="mt-1">
                        <strong>步骤：</strong
                        >{{ ensureArray(r.steps).join(" ") }}
                      </p>

                      <div
                        v-if="r.rationale"
                        class="mt-2 bg-[#F5EDD8]/30 p-2 rounded text-sandalwood/80 border border-sandalwood/10"
                      >
                        <div class="flex items-center gap-1 font-bold mb-1">
                          <BookOpen class="w-3 h-3" /> 医理依据
                        </div>
                        {{ r.rationale }}
                      </div>
                      <button
                        @click.stop="toggleFavorite(r)"
                        class="w-full mt-2 py-1 border border-dashed rounded flex justify-center gap-1"
                        :class="
                          favorited.has(String(r.id))
                      ...
                      >
                        <Heart
                          class="w-3 h-3"
                          :class="{
                            'fill-current': favorited.has(String(r.id)),
                          }"
                        />
                        {{ favorited.has(String(r.id)) ? "已收藏" : "收藏" }}
                      </button>
                    </div>
                  </div>

                  <div
                    v-if="m.acupoints && m.acupoints.length"
                    class="bg-gray-50 p-2.5 rounded border border-gray-100"
                  >
                    <div
                      class="text-xs font-bold text-sandalwood mb-1.5 flex items-center gap-1"
                    >
                      <span
                        class="w-1.5 h-1.5 bg-sandalwood rounded-full"
                      ></span>
                      穴位按揉
                    </div>
                    <div
                      v-if="
                        m.acupoints.length === 1 && m.acupoints[0].name === '无'
                      "
                    ...
                    <template v-else>
                      <div
                        v-for="(a, idx) in m.acupoints"
                        :key="idx"
                        class="text-xs mb-2 last:mb-0 border-b border-gray-200/50 pb-2 last:border-0 last:pb-0"
                      >
                        ...
                        <div
                          class="bg-white p-1.5 rounded text-sandalwood/80 border border-sandalwood/5"
                        >
                          👉 {{ a.method || "轻柔按压 3-5 分钟" }}
                        </div>
                      </div>
                    </template>
                  </div>
                  ...
                  <div
                    v-if="m.lifestyle && m.lifestyle.length"
                    class="text-xs text-gray-500 bg-gray-50 p-2 rounded"
                  >
                    <span class="font-bold text-sandalwood">🚫 禁忌：</span>
                    {{ (m.lifestyle || []).join("；") }}
                  </div>

                  <button
                    @click="handleSavePlan(m)"
                    class="w-full bg-sandalwood text-white py-2 rounded flex justify-center gap-2 text-sm hover:bg-sandalwood/90"
                  >
                    <PlusCircle class="w-4 h-4" /> 加入调理计划
                    (仅保存已收藏食谱)
                  </button>
                </div>
              </div>
          ...
          <div
            v-if="loadingStatus"
            class="mr-auto bg-[#F5EDD8] text-sandalwood px-4 py-2.5 rounded-lg text-sm flex items-center gap-2 animate-pulse"
          >
          ...
          <div
            v-if="
              prescriptionLoading &&
              prescriptionLoadingSessionId === activeSessionId
            "
            class="mr-auto w-[98%] space-y-2 animate-slide-up"
          >
            <div
              class="bg-white border border-sandalwood/20 rounded-xl overflow-hidden shadow-md animate-pulse"
            >
              <div class="bg-[#FAF6ED] p-3 border-b border-sandalwood/10">
            ...
            <div class="flex items-center gap-2 text-sandalwood text-xs">
              <Loader2 class="w-3.5 h-3.5 animate-spin" />正在生成处方...
            </div>
          </div>
        </div>
        ...
        <div class="p-3 border-t bg-[#FDFBF7] flex gap-2">
          <input
            ...
            :disabled="isGenerating"
            placeholder="输入症状..."
            class="flex-1 px-4 py-2 border rounded-lg outline-none focus:ring-1 focus:ring-sandalwood/50 disabled:opacity-50"
          />
          <button
            @click="send"
            :disabled="isGenerating"
            class="p-2 bg-sandalwood text-white rounded-lg disabled:opacity-50"
          >
            <Send class="w-5 h-5" />
          </button>
        </div>
