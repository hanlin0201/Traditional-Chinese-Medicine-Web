...
<template>
  <div class="w-full max-w-[95%] mb-4">
    ...
    >
      <div
        class="bg-[#F5EDD8] text-sandalwood border border-amber-200/60 px-4 py-3 rounded-2xl rounded-tl-none shadow-sm text-sm leading-relaxed"
      >
        {{ message.content }}
      </div>
    </div>
    ...
    <div v-else-if="message.role === 'user'" class="flex justify-end">
      <div
        class="bg-sandalwood text-white px-4 py-2 rounded-2xl rounded-tr-none shadow-sm text-sm"
      >
        {{ message.content }}
      </div>
    </div>
    ...
    <div
      v-else-if="message.type === 'inquiry'"
      class="flex justify-start w-full"
    >
      <div
        class="bg-white border border-sandalwood/20 rounded-xl p-4 shadow-sm w-full"
      >
        <p class="text-sandalwood text-sm mb-3 font-medium">
          {{ message.text }}
        </p>
    ...
    >
      <div
        class="bg-white border border-sandalwood/20 rounded-xl overflow-hidden shadow-md w-full"
      >
        ...
        <div class="p-3 space-y-3">
          ...
          >
            ...
            <div
              v-show="foldedStates[idx]"
              class="p-3 border-t border-gray-50 bg-[#FCFCFC] text-xs space-y-2 transition-all"
            >
              <div>
              ...
              <button
                ...
                :class="{
                  'text-red-500 bg-red-50 border-red-200': favorited.has(
                    recipe.name,
                  ),
                }"
              >
                <Heart
                  class="w-3 h-3"
                  :class="{ 'fill-current': favorited.has(recipe.name) }"
                />
