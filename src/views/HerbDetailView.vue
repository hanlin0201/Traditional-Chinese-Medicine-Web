...
import {
  ...
  setHerbEasyCache,
  profileCache,
} from "@/composables/usePagePreload";
...
const herbNatureMeridianHtml = computed(() =>
  formatNatureMeridianHtml(herbNatureMeridianRaw.value),
);
...
const getPartTagClass = (text) => {
  if (!text) return "";
  if (text.includes("根茎"))
    return "border-sandalwood/20 bg-sandalwood/10 text-sandalwood/90";
  if (text.includes("果实") || text.includes("种子"))
  ...
  if (text.includes("枝叶") || text.includes("树皮"))
    return "border-emerald-200 bg-emerald-50 text-emerald-700";
  if (text.includes("动物"))
  ...
  if (text.includes("其他"))
    return "border-sandalwood/15 bg-paper text-sandalwood";
  return "border-sandalwood/20 bg-sandalwood/5 text-sandalwood/90";
};
...
function formatTabooHtml(text) {
  ...
  if (body) {
    ...
    for (const line of lines) {
      ...
      const cls = isNum
        ? "herb-kai-note herb-rich-p"
        : "herb-rich-p text-sandalwood/90";
...
function formatClassicListHtml(text) {
  ...
  formatted = formatted.replace(
    /(\([1-9]\d?\))/g,
    '<br/><span class="text-sandalwood/90 font-semibold">$1</span>',
  );
  formatted = formatted.replace(
    /([①②③④⑤⑥⑦⑧⑨⑩⑪⑫⑬⑭⑮⑯⑰⑱⑲⑳])/g,
    '<br/><span class="text-sandalwood/90 font-semibold">$1</span>',
...
const checkFavoriteStatus = async () => {
  ...
  if (cached) {
    isFavorite.value = cached.some(
      (h) => h.id === herb.value.id || h.name === herb.value.name,
    );
    return;
  }
...
const toggleFavorite = async () => {
  ...
  try {
    if (isFavorite.value) {
      ...
      // --- 执行收藏 ---
      const { error } = await supabase.from("favorite_herbs").insert({
...
<template>
  <div class="min-h-screen pb-24 flex flex-col bg-[#FDFBF7]">
    <header
      class="sticky top-0 z-10 glass-search wood-overlay px-4 py-2 flex items-center justify-between border-b border-sandalwood/10"
    >
      <div class="flex items-center gap-3 overflow-hidden">
        ...
        >
          ...
          <span
            v-if="hasRecipeBackLink"
            class="text-sm font-medium font-serif truncate max-w-[14rem]"
          >
    ...
    <template v-if="loading">
      <div
        class="flex-1 flex flex-col items-center justify-center text-sandalwood/60 gap-3 min-h-[50vh]"
      >
        <div
          class="animate-spin rounded-full h-8 w-8 border-b-2 border-sandalwood"
        ></div>
    ...
    <template v-else-if="error || !herb">
      <div
        class="flex-1 flex flex-col items-center justify-center text-cinnabar gap-2 min-h-[50vh]"
      >
        <span class="text-3xl">🍂</span>
        <span class="font-serif">未找到【{{ route.params.name }}】的记录</span>
        <button @click="goBack" class="text-sm underline text-sandalwood mt-2">
          返回上一页
        </button>
      </div>
    </template>
    ...
    <template v-else>
      <!-- 顶部封面区域：统一纸张渐变 + 分类底图 -->
      <div
        class="w-full h-[220px] sm:h-[260px] md:h-[300px] bg-gradient-to-b from-[#F8F1E6] to-[#F1E4D1] border-b border-sandalwood/10 relative overflow-hidden"
      >
        <!-- 分类背景图：根据入药部位切换 -->
        <img
          v-if="herbPartBg"
        ...
        <div
          class="absolute inset-x-0 top-0 h-16 bg-gradient-to-b from-[#FDFBF7] via-[#FDFBF7]/40 to-transparent pointer-events-none"
        />
        <div
          class="absolute inset-x-0 bottom-0 h-20 bg-gradient-to-t from-[#FDFBF7] via-[#FDFBF7]/40 to-transparent pointer-events-none"
        />
      </div>
      ...
      <main class="flex-1 px-4 sm:px-6 pb-10 -mt-10 sm:-mt-16 relative z-0">
        ...
        <section class="max-w-4xl mx-auto">
          ...
          >
            ...
            <div
              v-else
              class="w-24 h-24 sm:w-28 sm:h-28 rounded-2xl border border-dashed border-sandalwood/20 bg-paper/70 flex items-center justify-center text-[11px] text-sandalwood/50 flex-shrink-0"
            >
              无图片
            </div>
            ...
            <div class="flex-1 space-y-2">
              <div class="flex items-center gap-2">
                <div
                  class="flex flex-wrap items-baseline gap-x-3 gap-y-1 flex-1 min-w-0"
                >
                  <h1
                    class="font-serif font-semibold text-sandalwood text-xl sm:text-2xl"
                  >
                    {{ herb?.name || "药材详情" }}
                  </h1>
                  <p
                    v-if="herb?.pinyin || herb?.latin_name"
                    class="text-xs sm:text-sm text-sandalwood/60"
                  >
                    {{ herb?.pinyin
                    }}<span v-if="herb?.pinyin && herb?.latin_name"> · </span
                    >{{ herb?.latin_name }}
                  </p>
                </div>
                <button
              ...
              <div v-if="herbTagInfo" class="space-y-1.5 pt-1">
                <div class="flex flex-wrap gap-1.5 text-[11px] sm:text-xs">
                  ...
                  <span
                    ...
                    :class="[
                      ...
                      getNatureTagClass(herbTagInfo.nature),
                    ]"
                  >
                    {{ herbTagInfo.nature }}
                  </span>
                  ...
                  <span
                    ...
                    :class="[
                      'px-2.5 py-1 rounded-full border font-semibold',
                      getPartTagClass(herbTagInfo.part),
                    ]"
                  >
                    {{ herbTagInfo.part }}
                  </span>
                ...
                <div
                  v-if="
                    herbTagInfo.detailEffects &&
                    herbTagInfo.detailEffects.length
                  "
              ...
              <!-- 关键属性一行展示（不再重复“某某类”） -->
              <div
                class="flex flex-wrap gap-1.5 mt-1 text-[11px] sm:text-xs"
              ></div>
            </div>
          </div>
        </section>
        ...
        <section class="max-w-4xl mx-auto mt-5 space-y-4">
          ...
          <div class="flex justify-center">
            <div
              class="flex p-1 rounded-xl bg-sandalwood/5 border border-sandalwood/10 w-full max-w-xs shadow-inner"
            >
              <button
                type="button"
                :class="[
        ...
        <div class="max-w-4xl mx-auto w-full mt-4 space-y-4 sm:space-y-5">
          ...
          <template v-if="detailMode === 'professional'">
            <section
              id="section-basic"
              class="rounded-xl bg-paper-card shadow-paper p-5 sm:p-6 border border-sandalwood/10"
            >
              <h2
                class="text-cinnabar font-serif font-semibold text-base mb-3 flex items-center gap-2"
              >
            ...
            <section
              id="section-taste-meridian"
              class="rounded-xl bg-paper-card shadow-paper p-5 sm:p-6 border border-sandalwood/10"
            >
              <h2
                class="text-cinnabar font-serif font-semibold text-base mb-2 flex items-center gap-2"
              >
            ...
            >
              ...
              <p
                v-if="herb.efficacy"
                class="text-sandalwood/90 text-sm leading-relaxed text-justify mb-2 font-medium"
              >
                {{ herb.efficacy }}
              </p>
              <div
            ...
            >
              <h2
                class="text-bamboo font-serif font-semibold text-base mb-2 flex items-center gap-2"
              >
          ...
          <template v-else-if="detailMode === 'easy'">
            <div
              v-if="!herbEasy"
              class="rounded-xl bg-paper-card shadow-paper p-8 border border-sandalwood/10 text-center text-sandalwood/60 text-sm font-serif"
            >
            ...
            <template v-else>
              <div class="space-y-4">
                <!-- 作用效果：主角卡片 -->
                <div
                  v-if="herbEasy.friendly_explanation"
                  class="rounded-xl bg-orange-210/70 border border-stone-300/50 p-6 relative overflow-hidden"
                >
                  <div
                    class="absolute top-0 left-0 w-1 h-full bg-amber-800/40"
                    aria-hidden="true"
                  />
                  <h2
                    class="text-stone-800 font-serif font-bold text-lg mb-3 flex items-center gap-2"
                  >
                    <span
                      class="w-1.5 h-5 bg-amber-800/45 rounded-full shrink-0"
                    />
                    作用效果
                  </h2>
                  <p
                    class="text-sandalwood text-base font-medium leading-relaxed text-justify"
                  >
                    {{ herbEasy.friendly_explanation }}
                  </p>
                </div>

                <div
                  class="rounded-xl bg-paper-card shadow-paper border border-sandalwood/10 divide-y divide-sandalwood/10"
                >
                  <div
                    v-if="
                      herbEasy.modern_scene != null &&
                      (Array.isArray(herbEasy.modern_scene)
                        ? herbEasy.modern_scene.length
                        : true)
                    "
                    class="p-4"
                  >
                    <h2
                      class="text-bamboo font-serif font-semibold text-sm mb-1.5 flex items-center gap-2"
                    >
                      <span class="w-1 h-3.5 bg-bamboo rounded" /> 应用场景
                    </h2>
                    <p
                      v-if="Array.isArray(herbEasy.modern_scene)"
                      class="text-sandalwood/80 text-xs leading-relaxed text-justify"
                    >
                      {{ herbEasy.modern_scene.join("；") }}
                    </p>
                    <p
                      v-else-if="typeof herbEasy.modern_scene === 'string'"
                      class="text-sandalwood/90 text-sm leading-relaxed text-justify"
                    >
                ...
                <section
                  ...
                  v-if="
                    ...
                    herbEasy.is_safe_for_pregnant !== null
                  "
                  class="rounded-xl bg-red-50/40 shadow-paper p-5 border border-red-100 space-y-3"
                >
                  <h2
                    class="text-cinnabar font-serif font-semibold text-base flex items-center gap-2"
                  >
                    <span class="w-1 h-4 bg-cinnabar rounded" /> 注意事项
                  </h2>

                  <p
                    v-if="herbEasy.safety_alarm"
                    class="text-sandalwood/90 text-sm leading-relaxed text-justify"
                  >
                    {{ herbEasy.safety_alarm }}
                  </p>
                  ...
                  <div
                    v-if="herbEasy.is_safe_for_pregnant !== null"
                    :class="{
                      'pt-3 border-t border-red-100': herbEasy.safety_alarm,
                    }"
                    class="flex items-center gap-2"
                  >
                    <span
                      class="text-sandalwood/70 text-sm shrink-0 font-medium"
                      >孕产妇说明：</span
                    >
                    <span
                      :class="
                        herbEasy.is_safe_for_pregnant
                          ? 'text-green-600'
                          : 'text-cinnabar'
                      "
...
.herb-rich :deep(.herb-kai-note) {
  font-family:
    KaiTi, "Kaiti SC", STKaiti, "STKaiti", "FangSong", "Noto Serif SC", serif;
  font-size: 0.8125rem;
  line-height: 1.7;
